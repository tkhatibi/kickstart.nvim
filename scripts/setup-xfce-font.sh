...
font_dir="$HOME/.fonts"
xfce_config_dir="$HOME/.config/xfce4/terminal"
xfce_config_file="$xfce_config_dir/terminalrc"
...
if [ -f "$xfce_config_file" ]; then
  ...
  mkdir $xfce_config_dir -p
  touch $xfce_config_file
  echo "Created Xfce Terminal config file: $xfce_config_file"
fi
